...
- **[ENTRY] Pipeline rooms vs existing supply** (Riyadh): `18.3`. Pause market development. Activate H2: redirect to next-priority country.
- **[ENTRY] Pipeline rooms vs existing supply** (Casablanca): `16.1`. Pause market development. Activate H2: redirect to next-priority country.
- **[ENTRY] Forward booking pace vs prior year** (Casablanca): `78.0`. Demand pace below 80% prior year. Reassess opening-year P&L.
...
- **A5 — ✗ AT RISK**: Loyalty programme retains share against rival devaluations
  ...
  - _Action: Brief loyalty team; consider targeted status-match campaign within 30 days._

## 3. Hedging actions triggered

- **H2 — Deprioritise market; redirect dev effort**
  - Supply pipeline exceeds 15% of existing stock — oversupply risk. Owner: Development Director. Action: Deprioritise market in pipeline review; redirect BD effort to next-priority country on target list. Deadline: next pipeline review meeting.
- **H1 — Pause LOI; add FX floor clause**
  - FX depreciation exceeds 15% against EUR. Owner: Development Director. Action: Pause all LOI negotiations in affected market; add FX floor clause to any term sheet currently in negotiation. Deadline: before next LOI signature.

---
_Brief generated without LLM (ANTHROPIC_API_KEY not set). Set the env var to receive a Claude-authored synthesis._